...
int
execute_multi_command(cmd_line *line, job *job_list, int first_cpid, job *cur_job, int *left_pipe,int *right_pipe ) {
    int fd[2];
    int cpid = 0;
    int cur_pgid = first_cpid;
    cpid = fork();
    ...
    if (line->next) {
        ...
        execute_multi_command(line->next,job_list,cur_pgid,cur_job,right_pipe,fd);
    }
    return 0;
}
        ...
        if (strcmp(com, "\n") != 0) {
            if (strcmp(com, "jobs\n") == 0) {
                ...
                if (command->next == 0) {
                    ...
                    execute_multi_command(command, job_list, -1, cur_job,NULL,fd);
                    if (command->blocking == 1) {
                        waitpid(cur_job->pgid, &cur_job->status, WUNTRACED);
                    }
                    free_cmd_lines(command);
