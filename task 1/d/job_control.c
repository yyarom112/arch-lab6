...
job* find_job_by_index(job* job_list, int idx){
	...
	fprintf(stderr, "There is no job with such an index!\n");
	return NULL;
}
    ...
    switch (j->status) {
        ...
        default:
            ...
            if(cont==1 && j->status==SUSPENDED){
                //set the attributes of the terminal to that of the job's using
            ...
            // the process group receives: a SIGTSTP (ctrl-z)
            ...
            //Check for status update of jobs and remove the done jobs
...
void run_job_in_background (job *j, int cont){
	job* job = find_job_by_index(j, cont);
	if(job !=NULL){
		job->status = RUNNING;
